      ...
      <section className="checkout-shell">
        ...
        <div className="checkout-layout">
          <form id="checkout-form" className="checkout-form" onSubmit={step === 1 ? (event) => { event.preventDefault(); if (!items.length) return; if (purchaseMode === "combo" && !appliedPromotion) { setCouponMessage("Hãy áp dụng một combo hợp lệ trước khi tiếp tục."); return; } setStep(2); } : step === 2 ? continueFromShipping : submitOrder}>
            ...
            {step === 3 && <div className="checkout-stage"><div className="checkout-panel-title"><span>03</span><div><h2>Phương thức thanh toán</h2><p>Chọn cách thanh toán thuận tiện cho bạn.</p></div></div><div className="payment-options"><label className={paymentMethod === "cod" ? "selected" : ""}><input type="radio" name="payment" value="cod" checked={paymentMethod === "cod"} onChange={() => setPaymentMethod("cod")} /><div className="payment-option-main"><span className="payment-option-icon payment-option-icon-cod">◫</span><div className="payment-option-copy"><b>Thanh toán khi nhận hàng</b><small>Kiểm tra hàng trước khi thanh toán</small></div></div><strong>COD</strong></label><label className={paymentMethod === "bank" ? "selected" : ""}><input type="radio" name="payment" value="bank" checked={paymentMethod === "bank"} onChange={() => setPaymentMethod("bank")} /><div className="payment-option-main"><span className="payment-option-icon payment-option-icon-bank">🏦</span><div className="payment-option-copy"><b>Chuyển khoản ngân hàng</b><small>Thông tin tài khoản hiển thị sau khi đặt hàng</small></div></div><div className="payment-option-side"><strong>ATM</strong><span className="payment-option-arrow">›</span></div></label><label className={paymentMethod === "vnpay" ? "selected" : ""}><input type="radio" name="payment" value="vnpay" checked={paymentMethod === "vnpay"} onChange={() => setPaymentMethod("vnpay")} /><div className="payment-option-main"><span className="payment-option-icon payment-option-icon-wallet">💳</span><div className="payment-option-copy"><b>Ví điện tử / VNPay</b><small>Thanh toán nhanh và bảo mật</small></div></div><div className="payment-option-side payment-option-side-logos"><div className="payment-option-brand-row"><span className="brand-badge brand-momo">MoMo</span><span className="brand-badge brand-zalopay">ZaloPay</span><span className="brand-badge brand-viettel">ViettelPay</span></div><strong>ONLINE</strong></div></label><label className={paymentMethod === "card" ? "selected" : ""}><input type="radio" name="payment" value="card" checked={paymentMethod === "card"} onChange={() => setPaymentMethod("card")} /><div className="payment-option-main"><span className="payment-option-icon payment-option-icon-card">◫</span><div className="payment-option-copy"><b>Thẻ tín dụng / ghi nợ</b><small>Visa, Mastercard, JCB</small></div></div><div className="payment-option-side payment-option-side-logos"><div className="payment-option-brand-row"><span className="brand-badge brand-visa">VISA</span><span className="brand-badge brand-jcb">JCB</span></div></div></label></div><div className="checkout-navigation"><button type="button" onClick={() => setStep(2)}>← Quay lại</button><button className="checkout-next" type="submit">Xác nhận đặt hàng <span>✓</span></button></div></div>}
          ...
          <aside className="checkout-summary">
            ...
            <div className="checkout-items">{items.length ? items.map((item) => <div className="checkout-item" key={`${item.product.id}-${item.weight}`}><div className="checkout-item-image"><img src={item.product.image} alt="" /></div><div><strong>{item.product.name}</strong><small>{item.weight}</small></div><div className="checkout-item-meta"><b>×{item.quantity}</b><span>{formatPrice(item.product.price * item.quantity)}</span></div></div>) : <p className="checkout-empty">Giỏ hàng của bạn đang trống.</p>}</div>
