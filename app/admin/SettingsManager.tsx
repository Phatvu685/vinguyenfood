...
function ImageField({ label, value, onChange }: { label: string; value: string; onChange: (value: string) => void }) {
  ...
  function upload(event: ChangeEvent<HTMLInputElement>) {
    ...
    <div className="admin-image-field"><span>{label}</span><div className="admin-image-upload"><div className="vg-upload-source"><button type="button" className={source === "upload" ? "active" : ""} onClick={() => { setSource("upload"); onChange(""); }}>Upload ảnh</button><button type="button" className={source === "url" ? "active" : ""} onClick={() => setSource("url")}>URL ảnh</button></div>{source === "upload" ? <label className="admin-image-upload-button">Tải ảnh<input type="file" accept="image/png,image/jpeg,image/webp,image/svg+xml" onChange={upload} /></label> : <input type="text" value={value} onChange={event => onChange(event.target.value)} placeholder="Nhập URL hoặc tải ảnh lên..." />}</div>{value && <img className="admin-image-preview" src={value} alt={`Xem trước ${label.toLowerCase()}`} />}</div>;
}
