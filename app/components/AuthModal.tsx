    ...
    <div className="auth-overlay" role="presentation" onMouseDown={onClose}>
      <section className="auth-modal" role="dialog" aria-modal="true" aria-labelledby="auth-title" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <div className="auth-content">
          ...
          <form className="auth-form" onSubmit={submit} onInput={() => formError && setFormError("")}>
            ...
            <label><span>{mode === "login" ? "Số điện thoại hoặc email admin" : "Số điện thoại"}</span><span className="auth-input"><i>☏</i><input name="phone" type={mode === "login" ? "text" : "tel"} placeholder={mode === "login" ? "0901 234 567 hoặc admin@vigenfood.com" : "0901 234 567"} autoComplete={mode === "login" ? "username" : "tel"} pattern={mode === "login" ? undefined : "^(0|\+84)[0-9]{9,10}$"} required /></span></label>
            <label><span>Mật khẩu</span><span className="auth-input password-field"><i>⌑</i><input name="password" type={showPassword ? "text" : "password"} placeholder="Ít nhất 6 ký tự" autoComplete={mode === "login" ? "current-password" : "new-password"} minLength={6} required /><button type="button" className="password-toggle" onClick={() => setShowPassword(!showPassword)} aria-label={showPassword ? "Ẩn mật khẩu" : "Hiện mật khẩu"}><EyeIcon open={showPassword} /></button></span></label>
